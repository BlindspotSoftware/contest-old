...
func (ts *TestStep) powerOn(ctx xcontext.Context, builder *strings.Builder, target *target.Target, ev testevent.Emitter) error {
	...
	writeCommandOutput(builder, "DUT was powered on successfully.")

	return nil
}

// powerOffSoft turns off the device.
func (ts *TestStep) powerOffSoft(ctx xcontext.Context, builder *strings.Builder, target *target.Target, ev testevent.Emitter) error {
	...
	writeCommandOutput(builder, "DUT was powered off successfully.")

	return nil
}

// powerOffHard ensures that -> pdu is off & reset is on.
func (ts *TestStep) powerOffHard(ctx xcontext.Context, builder *strings.Builder, target *target.Target, ev testevent.Emitter) error {
	if err := ts.resetDUT(ctx); err != nil {
		return fmt.Errorf("Failed to reset DUT: %v", err)
	}

	writeCommandOutput(builder, "DUT was resetted successfully.")
