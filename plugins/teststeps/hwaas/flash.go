...
func (ts *TestStep) flashWrite(ctx xcontext.Context, builder *strings.Builder, arg string, target *target.Target, ev testevent.Emitter) error {
	if arg == "" {
	...
	writeCommandOutput(builder, "DUT is flashed successfully.")

	return nil
}

// flashRead executes the flash read command.
func (ts *TestStep) flashRead(ctx xcontext.Context, builder *strings.Builder, arg string, target *target.Target, ev testevent.Emitter) error {
	if arg == "" {
	...
	writeCommandOutput(builder, "DUT flash was read successfully.")
