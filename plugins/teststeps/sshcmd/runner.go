...
func (r *TargetRunner) Run(ctx xcontext.Context, target *target.Target) error {
	...
	_, err = r.runCMD(ctx, &stdoutMsg, &stderrMsg, target, transport, params)
	if err != nil {
...
func (r *TargetRunner) runCMD(ctx xcontext.Context, stdoutMsg, stderrMsg *strings.Builder, target *target.Target,
	...
	proc, err := transport.NewProcess(ctx, params.Bin.Executable, params.Bin.Args)
	if err != nil {
...
func parseOutput(stdoutMsg, stderrMsg *strings.Builder, stdout []byte, expects []Expect) error {
	...
	for _, expect := range expects {
		...
		if err != nil {
			...
			stderrMsg.WriteString(fmt.Sprintf("%s", err.Error()))
		}
